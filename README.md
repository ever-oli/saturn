...
1. Derive six face textures from the upload (`single` / `wrap` / `grid` / `auto`) — see `geometry/faces.py`.
2. Build a Latin-cross net with crisp panel borders (`geometry/net.py`).
3. Render an approximate isometric cube with the same faces (`geometry/cube.py`).
4. Place the cube on the chest and the net on the back of blank tee templates (`geometry/mockup.py`).
...
| `wrap` | Horizontal equator band around LEFT → FRONT → RIGHT → BACK so a trim (gold band, horizon) stays continuous. |
| `grid` | 2×3 crop grid, one cell per face. |
| `auto` | `wrap` if the source is wide (aspect ≥ 1.4), else `single`. |
