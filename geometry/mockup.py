...
from geometry.types import flatten_rgb
...
def composite_shirts(
    ...
    Returned front/back/combined are opaque RGB so Gradio never shows a
    transparency checker around the shirt or through the graphics.
    """
    ...
    return ShirtMockup(
        combined=combined,
        front=flatten_rgb(front, background),
        back=flatten_rgb(back, background),
    )
...
def _side_by_side(
    ...
    sheet.paste(front_r, (pad, pad), front_r.convert("RGBA"))
    sheet.paste(back_r, (pad + front_r.width + gap, pad), back_r.convert("RGBA"))
    return sheet
